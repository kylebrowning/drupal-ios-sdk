//
//  DrupalViewTableViewController.swift
...
/// want their own `Decodable` row type — provide one to
/// ``DrupalViewTableViewController`` via its generic `Row` parameter.
...
@available(iOS 15.0, *)
open class DrupalViewTableViewController<Row: Decodable, Cell: UITableViewCell>: UITableViewController {

    public var rows: [Row] = []
    public let reuseIdentifier = "Cell"
    public let configure: (Cell, Row) -> Void
    public var didSelect: (Row) -> Void = { _ in }
    public var didFinish: ([Row], DrupalViewTableViewController) -> Void = { _, _ in }
    ...
    public convenience init(viewPath: String, configure: @escaping (Cell, Row) -> Void) {
        ...
        Task { @MainActor in
            do {
                let data = try await Drupal.shared.view(at: viewPath)
                let items = try JSONDecoder().decode([Row].self, from: data)
                self.rows = items
                self.tableView.reloadData()
                self.didFinish(items, self)
            } catch {
                print("DrupalIOSSDK error fetching view: \(error.localizedDescription)")
                self.didFinish([], self)
            }
        }
    }
